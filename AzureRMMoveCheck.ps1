...
$Azureresources = Get-AzureRmResource | select ResourceId, Name, ResourceType
"Subscription,Resource Name,Resource Type,Move Subscription" |Out-File -FilePath .\results.csv
foreach($resource in $Azureresources){
    foreach($support in $moveSupported){
        ...
        if($resource."ResourceType" -like "*"+$test+"*") {
            write-host $resource.resourceid.Split("/")[2] "," $resource.name "," $support."Move Subscription"
